    ...
    for file in files:
        if jit or jit_no_reg_alloc:
            ...
            if filename in JIT_EXCLUDE_FILES:
                continue
        def setlimits():
            resource.setrlimit(resource.RLIMIT_STACK, (40960, 40960))
        ...
        if len(qemu) == 0:
            proc = Popen(subprocess_args, stdout=PIPE, stderr=PIPE, preexec_fn=setlimits)
            out, _ = proc.communicate()
            returncode = proc.returncode
        else:
            try:
                run(subprocess_args, check=True, stdout=PIPE, stderr=PIPE, preexec_fn=setlimits)
                returncode = 0
                out = ""
