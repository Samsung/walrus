...
def parse_args():
  parser = argparse.ArgumentParser(
      ...
      description="Run wasm benchmarks based on the benchmark game.",
      formatter_class=CustomHelpFormat,
  )
  # Not in constructor, since this feature was added in python 3.14
  parser.suggest_on_error = True

  parser.add_argument(
      "--test-dir",
      metavar="PATH",
      help="path to the test files written in c",
      nargs="?",
      const=TEST_DIR,
      default=TEST_DIR,
  )
  parser.add_argument("--only-game", help="only run Game tests",
                      action="store_true")
  parser.add_argument("--only-simd", help="only run SIMD tests",
                      action="store_true")
  parser.add_argument("--run", metavar="TEST", help="only run one benchmark")
  parser.add_argument(
      "--engines",
      metavar="PATH",
      help="paths to wasm engines",
      nargs="+",
      # default=["walrus"],
  )
  parser.add_argument(
  ...
  parser.add_argument(
      "--iterations",
      metavar="NUMBER",
      help="how many times run the tests, default: 4",
      nargs="?",
      const="4",
      default=4,
      type=int,
  ...
  parser.add_argument(
      "--compile-anyway",
      help="compile the tests even if they are compiled",
      action="store_true",
  )
  parser.add_argument("--mem", help="measure MAX RSS", action="store_true")
  parser.add_argument("--verbose", help="prints extra informations e.g. paths",
                      action="store_true")
  parser.add_argument(
      "--no-system-emcc",
      help="Don't use emcc command from the system, instead get it from github",
      action="store_true",
      default=False,
  )
  parser.add_argument("--no-time", help="Don't measure time",
                      action="store_true", default=False)
  parser.add_argument(
      "--results",
      choices=["i", "j", "n", "j2i", "i2j", "n2j", "j2n", "n2i", "i2n"],
      help="Type of results to show, seperated by spaces, default: i j n",
      nargs="*",
      default=["i", "j", "n"],
  )
  parser.add_argument(
      "--engine-path",
      help="Show engine path: -2 for serial number, -1 for hide, 0 for all, positive numbers for the levels from top.",
      type=int,
      default=-2,
  )
  parser.add_argument("--summary", help="Generate summary",
                      action="store_true", default=False)
  parser.add_argument("--arch",
                      help="what architecture the compiled walrus should have",
                      nargs="?")
  args = parser.parse_args()

  args.orig_results = args.results.copy()

  if ("j2i" in args.results or
      "i2j" in args.results) and "i" not in args.results:
    args.results.append("i")

  if ("j2i" in args.results or
      "i2j" in args.results) and "j" not in args.results:
    args.results.append("j")

  if ("n2i" in args.results or
      "i2n" in args.results) and "i" not in args.results:
    args.results.append("i")

  if ("n2i" in args.results or
      "i2n" in args.results) and "n" not in args.results:
    args.results.append("n")

  if ("n2j" in args.results or
      "j2n" in args.results) and "j" not in args.results:
    args.results.append("j")

  if ("n2j" in args.results or
      "j2n" in args.results) in args.results and "n" not in args.results:
    args.results.append("n")
...
def compile_tests(emcc_path, path, only_game, only_simd, compile_anyway, run,
                  verbose):
  ...
  for file in test_list:
    ...
    if not compile_anyway and os.path.exists(f"{path}/wasm/{name}.wasm"):
      if verbose:
        print(f"{name}.wasm is found; compilation skipped")
      continue
...
def run_wasm(engine, path, test_name, jit, jit_no_reg_alloc):
  ...
  flags = " --jit" if ((jit or jit_no_reg_alloc) and
                       "walrus" in engine) else ""
...
def measure_memory(path, name, engine, jit, jit_no_reg_alloc):
  ...
  outputs = subprocess.check_output(run_cmd, shell=True,
  ...
  for name in test_names:
    ...
    for i in range(number_of_runs):
      ...
      for engine in _engines:
        ...
        try:
          if not no_time:
            time_results[engine["name"]].append(
                measure_time(
                    path,
                    name,
                    run_wasm,
                    engine["path"],
                    engine["jit"],
                    engine["jit_no_reg_alloc"],
                ))
          if mem:
            mem_results[engine["name"]].append(
                measure_memory(
                    path,
                    name,
                    engine["path"],
    ...
    if not no_time:
      ...
      for engine in _engines:
        if sum(time_results[engine["name"]]) < 0:
          ...
          continue
        try:
          value = (sum(time_results[engine["name"]]) /
                   len(time_results[engine["name"]])) / 1e9
    ...
    if mem:
      ...
      for engine in _engines:
        if sum(mem_results[engine["name"]]) < 0:
          ...
          continue
        try:
          value = sum(mem_results[engine["name"]]) / len(
              mem_results[engine["name"]])
        except ZeroDivisionError:
          value = -1
...
def compare(
    ...
    engines,
    jit_to_interpreter,
    jit_no_reg_alloc_to_interpreter,
    jit_no_reg_alloc_to_jit,
    interpreter_to_jit,
    interpreter_to_jit_no_reg_alloc,
    jit_to_jit_no_reg_alloc,
):
  if (not jit_to_interpreter and not jit_no_reg_alloc_to_interpreter and
      not jit_no_reg_alloc_to_jit and not interpreter_to_jit and
      not interpreter_to_jit_no_reg_alloc and not jit_to_jit_no_reg_alloc):
    return
  for i in range(len(data)):
    ...
    for engine in engines:
      if jit_to_interpreter:
        ...
        data[i][f"{engine_display_name(engine)} INTERPRETER/JIT"] = (
            f"{jit} ({'{:.2f}'.format(-1 if float(interpreter) < 0 or float(jit) < 0 else float(interpreter) / float(jit))}x)"
        )

      if jit_no_reg_alloc_to_interpreter:
        jit_no_reg_alloc = data[i][
            f"{engine_display_name(engine)} JIT_NO_REG_ALLOC"]
        interpreter = data[i][f"{engine_display_name(engine)} INTERPRETER"]
        data[i][
            f"{engine_display_name(engine)} INTERPRETER/JIT_NO_REG_ALLOC"] = f"{jit_no_reg_alloc} ({'{:.2f}'.format(-1 if float(interpreter) < 0 or float(jit_no_reg_alloc) < 0 else float(interpreter) / float(jit_no_reg_alloc))}x)"

      if jit_no_reg_alloc_to_jit:
        jit_no_reg_alloc = data[i][
            f"{engine_display_name(engine)} JIT_NO_REG_ALLOC"]
        jit = data[i][f"{engine_display_name(engine)} JIT"]
        data[i][f"{engine_display_name(engine)} JIT/JIT_NO_REG_ALLOC"] = (
            f"{jit_no_reg_alloc} ({'{:.2f}'.format(-1 if float(jit) < 0 or float(jit_no_reg_alloc) < 0 else float(jit) / float(jit_no_reg_alloc))}x)"
        )

      if interpreter_to_jit:
        interpreter = data[i][f"{engine_display_name(engine)} INTERPRETER"]
        jit = data[i][f"{engine_display_name(engine)} JIT"]
        data[i][f"{engine_display_name(engine)} JIT/INTERPRETER"] = (
            f"{interpreter} ({'{:.2f}'.format(-1 if float(jit) < 0 or float(interpreter) < 0 else float(jit) / float(interpreter))}x)"
        )
      ...
      if interpreter_to_jit_no_reg_alloc:
        ...
        jit_no_reg_alloc = data[i][
            f"{engine_display_name(engine)} JIT_NO_REG_ALLOC"]
        data[i][
            f"{engine_display_name(engine)} JIT_NO_REG_ALLOC/INTERPRETER"] = f"{interpreter} ({'{:.2f}'.format(-1 if float(jit_no_reg_alloc) < 0 or float(interpreter) < 0 else float(jit_no_reg_alloc) / float(interpreter))}x)"

      if jit_to_jit_no_reg_alloc:
        jit = data[i][f"{engine_display_name(engine)} JIT"]
        jit_no_reg_alloc = data[i][
            f"{engine_display_name(engine)} JIT_NO_REG_ALLOC"]
        data[i][f"{engine_display_name(engine)} JIT_NO_REG_ALLOC/JIT"] = (
            f"{jit} ({'{:.2f}'.format(-1 if float(jit) < 0 or float(jit_no_reg_alloc) < 0 else float(jit_no_reg_alloc) / float(jit))}x)"
        )
  ...
  for test in data:
    ...
    for engine in engines:
      for result in orig_results:
        if result == "i":
          record[engine_display_name(engine) +
                 " INTERPRETER"] = test[engine_display_name(engine) +
                                        " INTERPRETER"]
        elif result == "j":
          record[engine_display_name(engine) +
                 " JIT"] = test[engine_display_name(engine) + " JIT"]
        elif result == "n":
          record[engine_display_name(engine) +
                 " JIT_NO_REG_ALLOC"] = test[engine_display_name(engine) +
                                             " JIT_NO_REG_ALLOC"]
        elif result == "j2i":
          record[engine_display_name(engine) +
                 " INTERPRETER/JIT"] = test[engine_display_name(engine) +
                                            " INTERPRETER/JIT"]
        elif result == "n2i":
          record[engine_display_name(engine) +
                 " INTERPRETER/JIT_NO_REG_ALLOC"] = test[
                     engine_display_name(engine) +
                     " INTERPRETER/JIT_NO_REG_ALLOC"]
        elif result == "n2j":
          record[engine_display_name(engine) + " JIT/JIT_NO_REG_ALLOC"] = (
              test[engine_display_name(engine) + " JIT/JIT_NO_REG_ALLOC"])
        elif result == "i2j":
          record[engine_display_name(engine) +
                 " JIT/INTERPRETER"] = test[engine_display_name(engine) +
                                            " JIT/INTERPRETER"]
        elif result == "i2n":
          record[engine_display_name(engine) +
                 " JIT_NO_REG_ALLOC/INTERPRETER"] = test[
                     engine_display_name(engine) +
                     " JIT_NO_REG_ALLOC/INTERPRETER"]
        else:
          record[engine_display_name(engine) + " JIT_NO_REG_ALLOC/JIT"] = (
              test[engine_display_name(engine) + " JIT_NO_REG_ALLOC/JIT"])
...
def getWalrus(args):
  if os.path.exists("./walrus"):
    os.system("(cd ./walrus && git fetch -a) >/dev/null")
    os.system("(cd ./walrus && git reset --hard origin/HEAD) >/dev/null")
  else:
    os.system(
        "git clone --depth 1 https://github.com/Samsung/walrus.git ./walrus >/dev/null"
    )
  os.system("(cd ./walrus && git submodule update --init)")

  arch = ""
  path = "./walrus/out/release/{}/walrus"
  if args.arch is None:
    arch = platform.machine()
  else:
    arch = args.arch

  cmake_string = "(cd ./walrus && cmake -DCMAKE_POLICY_VERSION_MINIMUM=3.5 -H. -Bout/release/{} -DWALRUS_ARCH={} -DWALRUS_HOST=linux -DWALRUS_MODE=release -DWALRUS_OUTPUT=shell -GNinja)"
  ninja_string = "(cd ./walrus && ninja -C ./out/release/{})"
  if "x86_64" in arch:
    arch = "x64"
  ...
  if os.path.exists("./walrus/out/release/{}/".format(arch)):
    os.system(
        "rm -r --interactive=never ./walrus/out/release/{}/".format(arch))
  os.system(cmake_string.format(arch, arch) + " &> /dev/null")
  os.system(ninja_string.format(arch))
  path = path.format(arch)
...
def main():
  ...
  if not args.no_time:
    ...
    result_data["time"] = orderData(result_data["time"], args.engines,
                                    args.orig_results)
    if args.report == None:
      print("# Time results\n")
  ...
  if args.mem:
    ...
    result_data["mem"] = orderData(result_data["mem"], args.engines,
                                   args.orig_results)
    if args.report == None:
