 ...
 public:
  ComponentAliasExport(bool is_core,
      ...
      : ComponentDef(is_core ? Alias::CoreExport : Alias::Export, sort),
        instance_index_(instance_index), export_name_(export_name), loc_(loc) {}
 ...
 public:
  ...
  class StringTable {
   public:
    ...
    const std::string* Find(const nonstd::string_view& name) const;
    const std::string* Append(const nonstd::string_view& name);
  ...
  Component(nonstd::string_view filename)
      : ComponentData(nullptr), filename(filename) {}

  nonstd::string_view Filename() const { return filename; }
