...
class BinaryReaderJIT : public BinaryReaderNop {
 ...
 private:
  struct FunctionType {
    FunctionType(Index param_count, Index result_count)
        : param_count(param_count), result_count(result_count) {}
...
Result BinaryReaderJIT::OnExport(Index index,
  ...
  switch (kind) {
    case ExternalKind::Func: {
      type = module_.func_types[item_index].Clone();
      break;
    }
    default: {
      return Result::Ok;
...
Result BinaryReaderJIT::OnLocalDecl(Index decl_index, Index count, Type type) {
  ...
  ValueInfo value_info = ValueLocation::typeToValueInfo(type);
...
Result BinaryReaderJIT::OnBinaryExpr(Opcode opcode) {
  ...
  compiler_.append(Instruction::Binary, opcode, 2,
                   ValueLocation::kFourByteSize);
  return Result::Ok;
}
...
Result BinaryReaderJIT::OnI32ConstExpr(uint32_t value) {
  ...
  Instruction* instr =
      compiler_.append(Instruction::Immediate, Opcode::I32Const, 0,
                       ValueLocation::kFourByteSize);
...
Result BinaryReaderJIT::OnIfExpr(Type sig_type) {
  ...
  compiler_.pushLabel(false);
  compiler_.appendBranch(Opcode::InterpBrUnless, 0);
  return Result::Ok;
}
...
Result BinaryReaderJIT::EndFunctionBody(Index index) {
  ...
  compiler_.append(Instruction::Any, Opcode::Return, func_type.results.size());

  printf("End compiling jit\n");

  compiler_.eliminateDeadCode();
