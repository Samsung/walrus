...
void JITCompiler::eliminateDeadCode() {
  ...
  while (item != nullptr) {
    if (item->isInstruction()) {
      ...
      if (instr->opcode() == Opcode::Br || instr->opcode() == Opcode::BrTable) {
        // Remove all instructions until a label is reached.
        item = item->next();

        while (true) {
          // A Return opcode must be present at the end.
          assert(item != nullptr);

          if (item->isLabel()) {
            if (item->asLabel()->branches().size() > 0) {
              break;
            }
          } else {
            Instruction* instr = item->asInstruction();
            ...
            if (instr->group() == Instruction::DirectBranch) {
              instr->value().target_label->remove(instr);
            } else if (instr->opcode() == Opcode::BrTable) {
              instr->asBrTable()->removeTargets();
            }
          }
      ...
      while (true) {
        ...
        InstructionListItem* prev = label->prev_;
        ...
        if (prev->isLabel()) {
          label->merge(prev->asLabel());
          remove(prev);
          continue;
        }

        Instruction* instr = prev->asInstruction();

        switch (instr->opcode()) {
          case Opcode::Br:
            if (instr->value().target_label == label) {
              label->remove(instr);
              remove(instr);
              continue;
            }

            if (instr->prev_ != nullptr && instr->prev_->isInstruction()) {
              Instruction* prev_instr = instr->prev_->asInstruction();

              if (prev_instr->isConditionalBranch() &&
                  prev_instr->value().target_label == label) {
                prev_instr->value().target_label = instr->value().target_label;
                instr->value().target_label->replace(instr, prev_instr);
                remove(instr);
                label->remove(prev_instr);

                if (prev_instr->opcode() == Opcode::BrIf) {
                  prev_instr->opcode_ = Opcode::InterpBrUnless;
                } else {
                  prev_instr->opcode_ = Opcode::BrIf;
                }
                continue;
              }
            }
          ...
          case Opcode::InterpBrUnless:
            if (instr->value().target_label == label) {
              label->remove(instr);
              replace(instr, new SimpleInstruction<1>(Instruction::Any,
                                                      Opcode::Drop, nullptr));
              continue;
            }
            break;
...
class ReduceMovesContext {
 public:
  struct StackItem {
    StackItem() : expected_group(WABT_JIT_INVALID_INSTRUCTION) {}
  ...
  struct InitGroups {
    InitGroups(Instruction* head_instr)
        : head_instr(head_instr), is_dead_group(false) {}
...
void ReduceMovesContext::update(std::vector<InstructionListItem*>& item_stack) {
  size_t size = item_stack.size();

  if (stack_.size() == 0) {
    stack_.resize(size);
  }

  assert(stack_.size() == size);

  for (size_t i = 0; i < size; i++) {
    InstructionListItem* item = item_stack[i];
    StackItem& stack_item = stack_[i];

    if (item == nullptr) {
      if (stack_item.expected_group != nullptr &&
          stack_item.expected_group != WABT_JIT_INVALID_INSTRUCTION) {
        stack_item.expected_group->combineInfo(Instruction::kKeepInstruction);
      }
    ...
    if (item->isInstruction()) {
      assert(stack_item.expected_group == nullptr ||
             stack_item.expected_group == WABT_JIT_INVALID_INSTRUCTION ||
             stack_item.expected_group == item);
      ...
      if (stack_item.expected_group == WABT_JIT_INVALID_INSTRUCTION) {
        ...
        item->asInstruction()->combineInfo(Instruction::kKeepInstruction);
        item_stack[i] = nullptr;
      }
      continue;
    }
...
Instruction* ReduceMovesContext::getHead(Instruction* instr) {
  assert(instr->group() == Instruction::LocalMove ||
         instr->group() == Instruction::Immediate);
...
void ReduceMovesContext::checkStack(Instruction* instr,
  ...
  Index local_index = head_instr->value().local_index;
  size_t end = stack.size();
  Opcode discarded = Opcode::LocalGet;

  if (instr->opcode() == Opcode::LocalSet) {
    discarded = Opcode::End;
  }
  ...
  for (size_t i = 0; i < end; i++) {
    ...
    if (stack_item != nullptr && stack_item->isInstruction()) {
      ...
      if (stack_instr->value().local_index == local_index &&
          stack_instr->opcode() != discarded) {
        stack_instr->combineInfo(Instruction::kKeepInstruction);
        stack[i] = nullptr;
      }
    }
  }
...
void ReduceMovesContext::InitGroups::updateLocalGet(Instruction* instr) {
  assert(head_instr == nullptr ||
         !(head_instr->info() & Instruction::kHasParent));
  ...
  if (is_dead_group) {
    // The instr is not Opcode::LocalGet, already has the
    // kKeepInstruction flag, or moved to head_instr.
    if (head_instr != nullptr) {
      head_instr->combineInfo(Instruction::kKeepInstruction);
      head_instr = nullptr;
    }
    return;
  }
  ...
  if (instr->value().local_index == head_instr->value().local_index) {
    // Combine only alive groups.
    instr->value().parent = head_instr;
    instr->combineInfo(Instruction::kHasParent);
    return;
  }

  is_dead_group = true;
  instr->combineInfo(Instruction::kKeepInstruction);
  head_instr->combineInfo(Instruction::kKeepInstruction);
  head_instr = nullptr;
}
...
void ReduceMovesContext::InitGroups::updateLocalSet(Instruction* instr,
  ...
  if (result->item == nullptr) {
    if (!is_dead_group && (instr->opcode() == Opcode::LocalGet ||
                           instr->group() == Instruction::Immediate)) {
      if (instr->info() & Instruction::kHasParent) {
        instr = ReduceMovesContext::getHead(instr);
      }
  ...
  if (!(instr->info() & Instruction::kKeepInstruction) &&
      instr->value().local_index == head_instr->value().local_index) {
    // Combine only alive groups.
    if (instr != head_instr) {
      instr->value().parent = head_instr;
      instr->combineInfo(Instruction::kHasParent);
...
void JITCompiler::reduceLocalAndConstantMoves() {
  assert(last_->asInstruction()->opcode() == Opcode::Return);

  if (last_->asInstruction()->paramCount() > 0) {
    Operand* operand = last_->asInstruction()->params();

    for (Index i = last_->asInstruction()->paramCount(); i > 0; --i) {
  ...
  for (InstructionListItem* item = first_; item != nullptr;
       item = item->next()) {
    if (item->isLabel()) {
      item->asLabel()->reduce_moves_ctx_ = new ReduceMovesContext();
      continue;
    }
  ...
  for (InstructionListItem* item = first_; item != nullptr;
    ...
    if (item->group() != Instruction::LocalMove ||
        item->asInstruction()->opcode() != Opcode::LocalSet) {
      continue;
    }
    ...
    const Label::DependencyList& deps =
        param->item->asLabel()->dependencies(param->index);
  ...
  for (InstructionListItem* item = first_; item != nullptr;
    ...
    if (item->isLabel()) {
      ...
      if (update_stack) {
        label->reduce_moves_ctx_->update(stack);
      }

      size_t size = label->reduce_moves_ctx_->stack()->size();
      stack.resize(size);
    ...
    if (instr->group() == Instruction::DirectBranch) {
      instr->value().target_label->reduce_moves_ctx_->update(stack);
  ...
  for (InstructionListItem* item = first_; item != nullptr;
    ...
    ReduceMovesContext* context = item->asLabel()->reduce_moves_ctx_;
    ...
    for (size_t i = 0; i < end; ++i) {
      ...
      while (!unprocessed_labels.empty()) {
        ...
        ReduceMovesContext::StackItem& current_item =
            label->reduce_moves_ctx_->stackItem(i);

        if (current_item.expected_group == nullptr) {
          item.expected_group = nullptr;

          if (expected_group != WABT_JIT_INVALID_INSTRUCTION) {
            expected_group->combineInfo(Instruction::kKeepInstruction);
          }
          break;
        }
  ...
  for (InstructionListItem* item = first_; item != nullptr;
       item = item->next()) {
    if (item->isLabel()) {
      std::vector<ReduceMovesContext::StackItem>* context_stack =
          item->asLabel()->reduce_moves_ctx_->stack();

      size_t size = context_stack->size();
      size_t start = size - item->asLabel()->dependencies_.size();
      ...
      for (size_t i = start; i < size; ++i) {
        ...
        if (expected_group != nullptr &&
            (expected_group->info() & Instruction::kKeepInstruction)) {
      ...
      delete item->asLabel()->reduce_moves_ctx_;
      continue;
    }
  ...
  while (item != nullptr) {
    ...
    if (instr->info() & Instruction::kHasParent) {
      ...
      assert((head_instr->info() & Instruction::kKeepInstruction) ||
             !(instr->info() & Instruction::kKeepInstruction));
    ...
    if (instr->opcode() != Opcode::LocalSet) {
      instr->getResult(0)->location.type = (instr->opcode() == Opcode::LocalGet)
                                               ? Operand::LocalGet
                                               : Operand::Unused;
      item = item->next();
      continue;
    ...
    if (!param->item->isLabel()) {
      ...
      assert(result->location.type == Operand::Stack ||
