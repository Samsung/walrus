...
class ReduceMovesContext {
 public:
  ...
  ReduceMovesContext(Label* label) : preserved_count_(label->preservedCount()) {
...
void JITCompiler::reduceLocalAndConstantMoves() {
  ...
  assert(last_ != nullptr && last_->isInstruction());

  if (last_->group() == Instruction::Return &&
      last_->asInstruction()->paramCount() > 0) {
  ...
  for (InstructionListItem* item = first_; item != nullptr;
    ...
    if (item->isLabel()) {
      ...
      label->reduce_moves_ctx_ = new ReduceMovesContext(label);
      continue;
    }
