...
Result BinaryReaderIR::OnRecursiveGroup(Index first_type_index,
                                        Index type_count) {
...
Result BinaryReaderIR::OnFuncType(Index index,
  ...
  auto func_type = MakeUnique<FuncType>(supertypes->is_final_sub_type);
...
Result BinaryReaderIR::OnStructType(Index index,
  ...
  auto field = MakeUnique<TypeModuleField>(GetLocation());
  auto struct_type =
      MakeUnique<StructType>(supertypes->is_final_sub_type);
...
Result BinaryReaderIR::OnArrayType(Index index,
                                   ...
                                   SupertypesInfo* supertypes) {
  auto field = MakeUnique<TypeModuleField>(GetLocation());
  auto array_type = MakeUnique<ArrayType>(supertypes->is_final_sub_type);
...
Result BinaryReaderIR::OnStartFunction(Index func_index) {
  ...
  module_->AppendField(
      MakeUnique<StartModuleField>(start, GetLocation()));
  return Result::Ok;
}
...
Result BinaryReaderIR::OnReturnCallExpr(Index func_index) {
  ...
  return AppendExpr(
      MakeUnique<ReturnCallExpr>(Var(func_index, GetLocation())));
}
...
Result BinaryReaderIR::OnLocalGetExpr(Index local_index) {
  return AppendExpr(
      MakeUnique<LocalGetExpr>(Var(local_index, GetLocation())));
}

Result BinaryReaderIR::OnI32ConstExpr(uint32_t value) {
  return AppendExpr(
      MakeUnique<ConstExpr>(Const::I32(value, GetLocation())));
}

Result BinaryReaderIR::OnI64ConstExpr(uint64_t value) {
  return AppendExpr(
      MakeUnique<ConstExpr>(Const::I64(value, GetLocation())));
}
...
Result BinaryReaderIR::OnLocalSetExpr(Index local_index) {
  return AppendExpr(
      MakeUnique<LocalSetExpr>(Var(local_index, GetLocation())));
}
...
Result BinaryReaderIR::OnLocalTeeExpr(Index local_index) {
  return AppendExpr(
      MakeUnique<LocalTeeExpr>(Var(local_index, GetLocation())));
}
...
Result BinaryReaderIR::OnQuaternaryExpr(Opcode opcode) {
  return AppendExpr(MakeUnique<QuaternaryExpr>(opcode));
}
...
Result BinaryReaderComponentIR::BeginComponent(uint32_t version, size_t depth) {
  ...
  auto component = MakeUnique<ComponentData>(def_list_->AsComponent());
...
Result BinaryReaderComponentIR::BeginCoreInstance(
  ...
  auto value_type =
      MakeUnique<ComponentInstance>(true, module_index, argument_count);
...
Result BinaryReaderComponentIR::OnAliasExport(
    ...
    const ComponentIndexLoc& instance_index,
    const ComponentStringLoc& name) {
  auto value = MakeUnique<ComponentAliasExport>(
      false, instance_index, string_table_.Append(name), sort, GetLocation());
...
Result BinaryReaderComponentIR::OnAliasOuter(ComponentSort sort,
                                             uint32_t counter,
                                             uint32_t index) {
  auto value = MakeUnique<ComponentAliasOuter>(counter, index, sort,
                                                     GetLocation());
  def_list_->AppendAny(std::move(value));
  return Result::Ok;
}
...
Result BinaryReaderComponentIR::OnPrimitiveType(const ComponentType& type) {
  ...
  auto value_type = MakeUnique<ComponentValueType>(
...
Result BinaryReaderComponentIR::OnListType(const ComponentTypeLoc& type) {
  auto value_type =
      MakeUnique<ComponentValueType>(ComponentTypeDef::List, type);
  def_list_->AppendType(std::move(value_type));
  return Result::Ok;
}
...
Result BinaryReaderComponentIR::OnListFixedType(const ComponentTypeLoc& type,
                                                uint32_t size) {
  auto value_type =
      MakeUnique<ComponentTypeListFixed>(type, size, GetLocation());
  def_list_->AppendType(std::move(value_type));
  return Result::Ok;
}
...
Result BinaryReaderComponentIR::OnOptionType(const ComponentTypeLoc& type) {
  auto value_type =
      MakeUnique<ComponentValueType>(ComponentTypeDef::Option, type);
  def_list_->AppendType(std::move(value_type));
  return Result::Ok;
}
...
Result BinaryReaderComponentIR::OnResultType(
  ...
  auto value_type =
      MakeUnique<ComponentTypeResult>(result_type, error_type);
  def_list_->AppendType(std::move(value_type));
  return Result::Ok;
}
...
Result BinaryReaderComponentIR::OnOwnType(const ComponentIndexLoc& index) {
  auto type =
      MakeUnique<ComponentTypeIndex>(ComponentTypeDef::Own, index);
...
Result BinaryReaderComponentIR::OnBorrowType(const ComponentIndexLoc& index) {
  auto type =
      MakeUnique<ComponentTypeIndex>(ComponentTypeDef::Borrow, index);
  def_list_->AppendType(std::move(type));
  return Result::Ok;
}
...
Result BinaryReaderComponentIR::OnStreamType(const ComponentTypeLoc& type) {
  auto value_type =
      MakeUnique<ComponentValueType>(ComponentTypeDef::Stream, type);
  def_list_->AppendType(std::move(value_type));
  return Result::Ok;
}
...
Result BinaryReaderComponentIR::OnFutureType(const ComponentTypeLoc& type) {
  auto value_type =
      MakeUnique<ComponentValueType>(ComponentTypeDef::Future, type);
  def_list_->AppendType(std::move(value_type));
  return Result::Ok;
}
...
Result BinaryReaderComponentIR::BeginInstanceType(uint32_t count) {
  auto type = MakeUnique<ComponentInterfaceType>(
...
Result BinaryReaderComponentIR::OnResourceType(ComponentResourceRep rep,
                                               const ComponentIndexLoc& dtor) {
  auto type = MakeUnique<ComponentTypeResource>(rep, dtor, GetLocation());
  def_list_->AppendType(std::move(type));
  return Result::Ok;
}
...
Result BinaryReaderComponentIR::OnResourceAsyncType(
  ...
  auto type = MakeUnique<ComponentTypeResource>(rep, dtor, callback,
                                                      GetLocation());
  def_list_->AppendType(std::move(type));
  return Result::Ok;
}
...
Result BinaryReaderComponentIR::OnCanonLift(
  ...
  auto canon =
      MakeUnique<ComponentCanonLift>(core_func_index, &opts, type_index);
...
Result BinaryReaderComponentIR::OnCanonLower(
  ...
  ComponentCanonOpts::OptionVector opts;
  ParseCanonOptions(option_count, options, &opts);
  auto canon = MakeUnique<ComponentCanonLower>(func_index, &opts);
  def_list_->Append(std::move(canon));
  return Result::Ok;
}
...
Result BinaryReaderComponentIR::OnCanonType(
    ...
    const ComponentIndexLoc& type_index) {
  auto canon_type = MakeUnique<ComponentCanonType>(canon, type_index);
