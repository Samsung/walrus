...
#define DEFINE_UNARY_BYTECODE_DUMP(name)                                                               \
    void dump(size_t pos)                                                                              \
    {                                                                                                  \
        printf(#name " src: %" PRIu32 " dst: %" PRIu32, (uint32_t)srcOffset(), (uint32_t)dstOffset()); \
    }
#else
#define DEFINE_UNARY_BYTECODE_DUMP(name)
#endif
...
class CallTable : public ByteCode {
public:
    CallTable(Opcode opcode, ByteCodeStackOffset calleeOffset, uint32_t tableIndex,
              FunctionType* functionType, uint16_t parameterOffsetsSize, uint16_t resultOffsetsSize)
        : ByteCode(opcode)
        , m_calleeOffset(calleeOffset)
    ...
    uint16_t parameterOffsetsSize() const { return m_parameterOffsetsSize; }
    uint16_t resultOffsetsSize() const { return m_resultOffsetsSize; }

    ByteCodeStackOffset* stackOffsets() const
    {
        return reinterpret_cast<ByteCodeStackOffset*>(reinterpret_cast<size_t>(this) + sizeof(CallTable));
    }
...
#define DEFINE_CALL_INDIRECT(className, opStr)                                                                                      \
    class className : public CallTable {                                                                                            \
    public:                                                                                                                         \
        className(ByteCodeStackOffset calleeOffset, uint32_t tableIndex, FunctionType* functionType,                                \
                  uint16_t parameterOffsetsSize, uint16_t resultOffsetsSize)                                                        \
            : CallTable(Opcode::className##Opcode, calleeOffset, tableIndex, functionType, parameterOffsetsSize, resultOffsetsSize) \
        {                                                                                                                           \
        }                                                                                                                           \
                                                                                                                                    \
        IF_DEBUG_ENABLED(                                                                                                           \
            void dump(size_t pos) {                                                                                                 \
                printf("%s tableIndex: %" PRId32 " ", opStr, m_tableIndex);                                                         \
                DUMP_BYTECODE_OFFSET(calleeOffset);                                                                                 \
                                                                                                                                    \
                size_t c = 0;                                                                                                       \
                auto arr = stackOffsets();                                                                                          \
                printf("paramOffsets: ");                                                                                           \
                for (size_t i = 0; i < parameterOffsetsSize(); i++) {                                                               \
                    printf("%" PRIu32 " ", (uint32_t)arr[c++]);                                                                     \
                }                                                                                                                   \
                printf(" ");                                                                                                        \
                                                                                                                                    \
                printf("resultOffsets: ");                                                                                          \
                for (size_t i = 0; i < resultOffsetsSize(); i++) {                                                                  \
                    printf("%" PRIu32 " ", (uint32_t)arr[c++]);                                                                     \
                }                                                                                                                   \
            })                                                                                                                      \
    };

DEFINE_CALL_INDIRECT(CallIndirect, "CallIndirect")
DEFINE_CALL_INDIRECT(CallIndirectM64, "CallIndirectM64")
...
DEFINE_CALL_INDIRECT(ReturnCallIndirect, "ReturnCallIndirect")
DEFINE_CALL_INDIRECT(ReturnCallIndirectM64, "ReturnCallIndirectM64")
...
#define DEFINE_STORE_32_BYTECODE(name, readType, writeType)                       \
    class name : public MemoryStore32 {                                           \
    public:                                                                       \
        name(uint32_t offset, ByteCodeStackOffset src0, ByteCodeStackOffset src1) \
            : MemoryStore32(Opcode::name##Opcode, offset, src0, src1)             \
        {                                                                         \
        }                                                                         \
        DEFINE_STORE_BYTECODE_DUMP(name)                                          \
    };

#define DEFINE_STORE_64_BYTECODE(name, readType, writeType)                       \
    class name : public MemoryStore64 {                                           \
    public:                                                                       \
        name(uint32_t offset, ByteCodeStackOffset src0, ByteCodeStackOffset src1) \
            : MemoryStore64(Opcode::name##Opcode, offset, src0, src1)             \
        {                                                                         \
        }                                                                         \
        DEFINE_STORE_BYTECODE_DUMP(name)                                          \
    };
...
#define DEFINE_STORE_MEMIDX_32_BYTECODE(name, readType, writeType)                                                    \
    class name : public MemoryStoreMemIdx32 {                                                                         \
    public:                                                                                                           \
        name(uint32_t index, uint32_t alignment, uint32_t offset, ByteCodeStackOffset src0, ByteCodeStackOffset src1) \
            : MemoryStoreMemIdx32(index, alignment, Opcode::name##Opcode, offset, src0, src1)                         \
        {                                                                                                             \
        }                                                                                                             \
        DEFINE_STORE_MEMIDX_BYTECODE_DUMP(name)                                                                       \
    };

#define DEFINE_STORE_MEMIDX_64_BYTECODE(name, readType, writeType)                                                    \
    class name : public MemoryStoreMemIdx64 {                                                                         \
    public:                                                                                                           \
        name(uint32_t index, uint32_t alignment, uint32_t offset, ByteCodeStackOffset src0, ByteCodeStackOffset src1) \
            : MemoryStoreMemIdx64(index, alignment, Opcode::name##Opcode, offset, src0, src1)                         \
        {                                                                                                             \
        }                                                                                                             \
        DEFINE_STORE_MEMIDX_BYTECODE_DUMP(name)                                                                       \
    };
...
#define DEFINE_TABLE_GET(className, opStr)                                                      \
    class className : public ByteCodeOffset2Value {                                             \
    public:                                                                                     \
        className(uint32_t index, ByteCodeStackOffset srcOffset, ByteCodeStackOffset dstOffset) \
            : ByteCodeOffset2Value(Opcode::className##Opcode, srcOffset, dstOffset, index)      \
        {                                                                                       \
        }                                                                                       \
                                                                                                \
        ByteCodeStackOffset srcOffset() const { return stackOffset1(); }                        \
...
DEFINE_TABLE_GET(TableGet, "")
DEFINE_TABLE_GET(TableGetM64, "M64")

#define DEFINE_TABLE_SET(className, opStr)                                            \
    class className : public ByteCodeOffset2Value {                                   \
    public:                                                                           \
        className(uint32_t index, ByteCodeStackOffset src0, ByteCodeStackOffset src1) \
            : ByteCodeOffset2Value(Opcode::className##Opcode, src0, src1, index)      \
        {                                                                             \
        }                                                                             \
                                                                                      \
        ByteCodeStackOffset src0Offset() const { return stackOffset1(); }             \
...
DEFINE_TABLE_SET(TableSet, "")
DEFINE_TABLE_SET(TableSetM64, "M64")

#define DEFINE_TABLE_GROW(className, opStr)                                                                    \
    class className : public ByteCode {                                                                        \
    public:                                                                                                    \
        className(uint32_t index, ByteCodeStackOffset src0, ByteCodeStackOffset src1, ByteCodeStackOffset dst) \
...
DEFINE_TABLE_GROW(TableGrow, "")
DEFINE_TABLE_GROW(TableGrowM64, "M64")

#define DEFINE_TABLE_SIZE(className, opStr)                              \
    class className : public ByteCodeOffsetValue {                       \
    public:                                                              \
        className(uint32_t index, ByteCodeStackOffset dst)               \
            : ByteCodeOffsetValue(Opcode::className##Opcode, dst, index) \
        {                                                                \
        }                                                                \
                                                                         \
...
DEFINE_TABLE_COPY(TableCopyM64, "M64")
DEFINE_TABLE_COPY(TableCopyM64M32, "M64M32")
DEFINE_TABLE_COPY(TableCopyM32M64, "M32M64")

#define DEFINE_TABLE_FILL(className, opStr)                                                                     \
    class className : public ByteCode {                                                                         \
    public:                                                                                                     \
        className(uint32_t index, ByteCodeStackOffset src0, ByteCodeStackOffset src1, ByteCodeStackOffset src2) \
            : ByteCode(Opcode::className##Opcode)                                                               \
            , m_tableIndex(index)                                                                               \
            , m_srcOffsets{ src0, src1, src2 }                                                                  \
        {                                                                                                       \
...
DEFINE_TABLE_FILL(TableFill, "")
DEFINE_TABLE_FILL(TableFillM64, "M64")

#define DEFINE_TABLE_INIT(className, opStr)                                                                                                 \
    class className : public ByteCode {                                                                                                     \
    public:                                                                                                                                 \
        className(uint32_t tableIndex, uint32_t segmentIndex, ByteCodeStackOffset src0, ByteCodeStackOffset src1, ByteCodeStackOffset src2) \
            : ByteCode(Opcode::className##Opcode)                                                                                           \
            , m_tableIndex(tableIndex)                                                                                                      \
            , m_segmentIndex(segmentIndex)                                                                                                  \
            , m_srcOffsets{ src0, src1, src2 }                                                                                              \
...
DEFINE_TABLE_INIT(TableInit, "");
DEFINE_TABLE_INIT(TableInitM64, "M64");
