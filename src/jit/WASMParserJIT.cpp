...
void WASMBinaryReaderJIT::OnExport(int kind, Index exportIndex, std::string name, Index itemIndex)
{
    ...
    m_module->m_exports.pushBack(new Walrus::ExportType(exportKind, new Walrus::String(name), itemIndex));
}
...
std::pair<Optional<Module*>, Optional<String*>> WASMParser::parseBinaryJIT(Store* store, const std::string& filename, const uint8_t* data, size_t len, int verboseLevel)
{
