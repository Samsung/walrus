...
#define SHORTIF(COND, VECTOR, FLOAT) FLOAT;
...
public:
    ...
    // clang-format off
#if (defined SLJIT_SEPARATE_VECTOR_REGISTERS && SLJIT_SEPARATE_VECTOR_REGISTERS)
    RegisterSet& vectorSet() { return m_vectorSet; }
#endif /* SLJIT_SEPARATE_VECTOR_REGISTERS */
    // clang-format on
            ...
            if ((*list & Instruction::FloatOperandMarker) && !(*list & Instruction::TmpNotAllowed)) {
                ...
                if ((*list & Instruction::TmpRequired) || (variable->info & VariableList::kIsImmediate)) {
                    ...
                    if (reg != VariableList::kUnusedReg) {
                        ASSERT(!(variable->info & VariableList::kIsImmediate));
                        SHORTIF(((*list & Instruction::TypeMask) == Instruction::V128Operand), regs.vectorReserve(reg), regs.floatReserve(reg))
#if (defined SLJIT_CONFIG_ARM_32 && SLJIT_CONFIG_ARM_32)
...
#endif /* SLJIT_CONFIG_ARM_32 */
                            SHORTIF(((*list & Instruction::TypeMask) == Instruction::V128Operand), reg = regs.vectorSet().allocateRegister(nullptr), reg = regs.floatSet().allocateRegister(nullptr))
#if (defined SLJIT_CONFIG_ARM_32 && SLJIT_CONFIG_ARM_32)
                        }
#endif /* SLJIT_CONFIG_ARM_32 */
                    }
                    SHORTIF(((*list & Instruction::TypeMask) == Instruction::V128Operand), instr->setRequiredReg(tmpIndex, regs.toCPUVectorReg(reg)), instr->setRequiredReg(tmpIndex, regs.toCPUFloatReg(reg)))
                }
                tmpIndex++;
            }
        ...
        if (hasResult) {
            ...
            if (type & Instruction::FloatOperandMarker) {
                if (resultVariable->reg1 != VariableList::kUnusedReg) {
                    SHORTIF((type == Instruction::V128Operand), regs.vectorReserve(resultVariable->reg1), regs.floatReserve(resultVariable->reg1))
#if (defined SLJIT_CONFIG_ARM_32 && SLJIT_CONFIG_ARM_32)
...
#endif /* SLJIT_CONFIG_ARM_32 */
                            SHORTIF((type == Instruction::V128Operand), resultReg = regs.vectorSet().allocateRegister(nullptr), resultReg = regs.floatSet().allocateRegister(nullptr))
#if (defined SLJIT_CONFIG_ARM_32 && SLJIT_CONFIG_ARM_32)
                        }
#endif /* SLJIT_CONFIG_ARM_32 */
                        SHORTIF((type == Instruction::V128Operand), resultReg = regs.toCPUVectorReg(resultReg), resultReg = regs.toCPUFloatReg(resultReg))
#if (defined SLJIT_32BIT_ARCHITECTURE && SLJIT_32BIT_ARCHITECTURE)
...
#endif /* SLJIT_CONFIG_ARM_32 */
                    SHORTIF((type == Instruction::V128Operand), regs.vectorReserve(resultReg), regs.floatReserve(resultReg))
                    SHORTIF((type == Instruction::V128Operand), resultReg = regs.toCPUVectorReg(resultReg), resultReg = regs.toCPUFloatReg(resultReg))
                } else {
#if (defined SLJIT_32BIT_ARCHITECTURE && SLJIT_32BIT_ARCHITECTURE)
                    if (type == Instruction::Int64Operand) {
        ...
        for (; *list != 0; list++) {
            ...
            if (instr->requiredReg(tmpIndex) == 0) {
                if (*list & Instruction::FloatOperandMarker) {
                    SHORTIF(((*list & Instruction::TypeMask) == Instruction::V128Operand), instr->setRequiredReg(tmpIndex, regs.toCPUVectorReg(regs.vectorSet().allocateRegister(nullptr))), instr->setRequiredReg(tmpIndex, regs.toCPUFloatReg(regs.floatSet().allocateRegister(nullptr))))
                } else {
    ...
    for (size_t i = 0; i < size; i++) {
        ...
        if (variable.reg1 != VariableList::kUnusedReg) {
            ...
            if (variable.info & Instruction::FloatOperandMarker) {
                SHORTIF((variable.info == Instruction::V128Operand), reg1 = regs.toCPUVectorReg(variable.reg1), reg1 = regs.toCPUFloatReg(variable.reg1))
            } else {
    ...
    for (InstructionListItem* item = m_first; item != nullptr; item = item->next()) {
        ...
        for (uint32_t i = 0; i < paramCount; i++) {
            ...
            if ((*list & Instruction::TypeMask & Instruction::FloatOperandMarker)
                ...
                // Source registers are read-only.
                if ((*list & Instruction::TmpRequired) || (variable.info & VariableList::kIsImmediate)) {
                    SHORTIF(((*list & Instruction::TypeMask) == Instruction::V128Operand), instr->setRequiredReg(tmpIndex, nextVectorIndex++), instr->setRequiredReg(tmpIndex, nextFloatIndex++))
#if (defined SLJIT_CONFIG_ARM_32 && SLJIT_CONFIG_ARM_32)
                    if ((*list & Instruction::TypeMask) == Instruction::V128Operand) {
