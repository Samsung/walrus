...
static void simdEmitOp(sljit_compiler* compiler, uint32_t opcode, sljit_s32 rd, sljit_s32 rn, sljit_s32 rm, uint32_t optype = 0)
{
    ...
    if ((optype & SimdOp::rn_gpreg) && rn >= SLJIT_R0) {
        rn = sljit_get_register_index(SLJIT_GP_REGISTER, rn);
    }
    if (!(optype & SimdOp::rm_imm) && !(optype & SimdOp::rm_gpreg) && rm >= SLJIT_VR0) {
        rm = sljit_get_register_index(SLJIT_SIMD_REG_128, rm);
    }
    if ((optype & SimdOp::rm_gpreg) && rm >= SLJIT_R0) {
...
static void simdEmitAbs(sljit_compiler* compiler, sljit_s32 type, sljit_s32 rd, sljit_s32 rn)
{
    ...
    simdEmitTypedOp(compiler, type, SimdOp::vmv_vv, tmp, 0, rn, SimdOp::rn_imm);
    simdEmitOp(compiler, SimdOp::vmslt_vx, mask, rn, TMP_ZERO);
    simdEmitOp(compiler, SimdOp::vrsub_vi ^ SimdOp::vm, tmp, rn, 0, SimdOp::rm_imm);
...
static void simdEmitAllTrue(sljit_compiler* compiler, sljit_s32 type, sljit_s32 rd, sljit_s32 rn)
{
    ...
    simdEmitTypedOp(compiler, type, SimdOp::vmv_vi, tmp, 0, (0x1F), SimdOp::rm_imm); //todo
    simdEmitOp(compiler, SimdOp::vredminu_vs, tmp, rn, tmp);
    simdEmitOp(compiler, SimdOp::vmv_xs, rd, 0, tmp, SimdOp::rn_imm | SimdOp::rd_gpreg);
...
static void simdEmitAnyTrue(sljit_compiler* compiler, sljit_s32 type, sljit_s32 rd, sljit_s32 rn) {
    sljit_s32 tmp = SLJIT_TMP_DEST_VREG;
    simdEmitTypedOp(compiler, type, SimdOp::vmv_sx, tmp, 0, TMP_ZERO, SimdOp::rm_gpreg);
...
static void simdEmitCompare(sljit_compiler* compiler, sljit_s32 type, uint32_t opcode, sljit_s32 rd, sljit_s32 rn, sljit_s32 rm, bool reverseMask = false)
{
    ...
    sljit_s32 tmp = SLJIT_TMP_DEST_VREG;
    simdEmitOp(compiler, SimdOp::vmv_vi, tmp, 0, reverseMask ? (0x1F) : 0, SimdOp::rm_imm | SimdOp::rn_imm); //todo
    simdEmitOp(compiler, SimdOp::vmerge_vi, rd, tmp, reverseMask ? 0 : (0x1F), SimdOp::rm_imm); //todo
}
...
static void simdEmitExtend(sljit_compiler* compiler, sljit_s32 type, bool low, bool s, sljit_s32 rd, sljit_s32 rn)
{
    ...
    if (low) {
        ...
        sljit_s32 t = SLJIT_SIMD_ELEM_8;
        uint32_t imm = 8;
        switch(type) {
            case SLJIT_SIMD_ELEM_32:
                t = SLJIT_SIMD_ELEM_16;
                imm = 4;
                break;
            case SLJIT_SIMD_ELEM_64:
                t = SLJIT_SIMD_ELEM_32;
                imm = 2;
                break;
        }
        simdEmitTypedOp(compiler, t, SimdOp::vslidedown_vi, tmp, rn, imm, SimdOp::rm_imm);
    }
    uint32_t opcode = s ? SimdOp::vsext_vf2 : SimdOp::vzext_vf2;
    opcode |= (sljit_get_register_index(SLJIT_SIMD_REG_128, rd) << 7) | (sljit_get_register_index(SLJIT_SIMD_REG_128, tmp) << 20);
    simdEmitVsetivli(compiler, type, 7);
    sljit_emit_op_custom(compiler, &opcode, sizeof(uint32_t));
}
...
static void simdEmitFCeil(sljit_compiler* compiler, sljit_s32 type, sljit_s32 rd, sljit_s32 rn)
{
    ...
    sljit_s32 mask = SLJIT_VR0;
    simdEmitTypedOp(compiler, type, SimdOp::vmv_vi, tmp, 0, 0, SimdOp::rm_imm | SimdOp::rn_imm);
    sljit_emit_op1(compiler, SLJIT_MOV, gptmp, 0, SLJIT_IMM, 64 - floatMantissaBits - floatExponentBits);
    simdEmitOp(compiler, SimdOp::vsll_vx, tmp, rn, gptmp, SimdOp::rm_gpreg);
    sljit_emit_op1(compiler, SLJIT_MOV, gptmp, 0, SLJIT_IMM, 64 - floatExponentBias);
    simdEmitOp(compiler, SimdOp::vsrl_vx, tmp, tmp, gptmp, SimdOp::rm_gpreg);
    sljit_emit_op1(compiler, SLJIT_MOV, gptmp, 0, SLJIT_IMM, floatExponentBias + floatMantissaBits);
    simdEmitOp(compiler, SimdOp::vmslt_vx, mask, tmp, gptmp, SimdOp::rm_gpreg);
    ...
    simdEmitOp(compiler, SimdOp::vfcvt_x_f_v ^ SimdOp::vm, rd, rn, 0, SimdOp::rm_imm);
    simdEmitOp(compiler, SimdOp::vfcvt_f_x_v ^ SimdOp::vm, rd, rd, 0, SimdOp::rm_imm);
    if (rd == rn) {
    ...
    simdEmitOp(compiler, SimdOp::vxor_vi, mask, mask, (0x1F), SimdOp::rm_imm); //todo
...
static void simdEmitFMinMax(sljit_compiler* compiler, sljit_s32 type, sljit_s32 opcode, sljit_s32 rd, sljit_s32 rn, sljit_s32 rm)
{
    ...
    sljit_emit_op1(compiler, SLJIT_MOV, gptmp, 0, SLJIT_IMM, type == (SLJIT_SIMD_FLOAT | SLJIT_SIMD_ELEM_64) ? 0x7FF8000000000000 : 0x7FC00000U /* NAN */);
    simdEmitOp(compiler, SimdOp::vmv_vx, tmp, 0, gptmp, SimdOp::rm_gpreg | SimdOp::rn_imm);
    simdEmitOp(compiler, opcode ^ SimdOp::vm, tmp, rn, rm);
    simdEmitOp(compiler, SimdOp::vmv_vv, rd, 0, tmp, SimdOp::rn_imm);
}

static void SimdEmitFTrunc(sljit_compiler* compiler, sljit_s32 type, sljit_s32 rd, sljit_s32 rn)
{
    sljit_s32 tmp = SLJIT_TMP_DEST_VREG;
    simdEmitTypedOp(compiler, type, SimdOp::vfcvt_x_f_v, tmp, rn, 0, SimdOp::rm_imm);
    simdEmitTypedOp(compiler, type, SimdOp::vfcvt_f_x_v, rd, tmp, 0, SimdOp::rm_imm);
...
{
#if defined (__riscv_zvbb)
    simdEmitTypedOp(compiler, type, SimdOp::vcpop_v, rd, rn, 0, SimdOp::rm_imm);
#else
    sljit_s32 mask = SLJIT_VR0;
    sljit_s32 tmp = SLJIT_TMP_DEST_VREG;
    sljit_s32 tmpgp = SLJIT_TMP_DEST_REG;
    simdEmitTypedOp(compiler, type, SimdOp::vmv_vv, tmp, 0, rn);
    simdEmitOp(compiler, SimdOp::vmv_vi, rd, 0, 0, SimdOp::rm_imm);
    struct sljit_label* label = sljit_emit_label(compiler);
    simdEmitOp(compiler, SimdOp::vmsne_vi, mask, tmp, 0);
    simdEmitOp(compiler, SimdOp::vadd_vi ^ SimdOp::vm, rd, rd, 1, SimdOp::rm_imm); //todo
    simdEmitOp(compiler, SimdOp::vadd_vi ^ SimdOp::vm, rt, tmp, (0x1F), SimdOp::rm_imm); //todo
...
// static void simdEmitI32X4TruncSatF64x2(sljit_compiler* compiler, sljit_s32 rd, sljit_s32 rn, sljit_s32 rm, bool s) {
//     sljit_s32 tmp = SLJIT_TMP_DEST_VREG;
//     sljit_s32 mask = SLJIT_VR0;
...
static void emitUnarySIMD(sljit_compiler* compiler, Instruction* instr)
{
    ...
    case ByteCode::F64X2ConvertLowI32X4UOpcode:
        break;
    case ByteCode::V128NotOpcode:
        simdEmitTypedOp(compiler, srcType, SimdOp::vxor_vv, dst, args[0].arg, (0x1F), SimdOp::rm_imm); //todo
        break;
    case ByteCode::I8X16NegOpcode:
    case ByteCode::I16X8NegOpcode:
    case ByteCode::I32X4NegOpcode:
    case ByteCode::I64X2NegOpcode:
        simdEmitTypedOp(compiler, srcType, SimdOp::vrsub_vi, dst, args[0].arg, 0);
        break;
    case ByteCode::I8X16AbsOpcode:
    case ByteCode::I16X8AbsOpcode:
    case ByteCode::I32X4AbsOpcode:
    ...
    case ByteCode::I32X4ExtendLowI16X8SOpcode:
    case ByteCode::I64X2ExtendLowI32X4SOpcode:
        simdEmitExtend(compiler, srcType, true, true, dst, args[0].arg);
        break;
    case ByteCode::I16X8ExtendHighI8X16SOpcode:
    case ByteCode::I32X4ExtendHighI16X8SOpcode:
    case ByteCode::I64X2ExtendHighI32X4SOpcode:
        simdEmitExtend(compiler, srcType, false, true, dst, args[0].arg);
        break;
    case ByteCode::I16X8ExtendLowI8X16UOpcode:
    case ByteCode::I32X4ExtendLowI16X8UOpcode:
    case ByteCode::I64X2ExtendLowI32X4UOpcode:
        simdEmitExtend(compiler, srcType, true, false, dst, args[0].arg);
        break;
    case ByteCode::I16X8ExtendHighI8X16UOpcode:
    case ByteCode::I32X4ExtendHighI16X8UOpcode:
    case ByteCode::I64X2ExtendHighI32X4UOpcode:
    ...
    case ByteCode::F32X4SqrtOpcode:
    case ByteCode::F64X2SqrtOpcode:
        simdEmitTypedOp(compiler, srcType, SimdOp::vfsqrt_v, dst, args[0].arg, 0);
        break;
    case ByteCode::F32X4TruncOpcode:
    case ByteCode::F64X2TruncOpcode:
...
static void emitShiftSIMD(sljit_compiler* compiler, Instruction* instr)
{
    ...
    simdEmitTypedOp(compiler, type, op, dst, args[0].arg, isImm ? args[1].argw : args[1].arg, isImm ? SimdOp::rm_imm : SimdOp::rm_gpreg);
