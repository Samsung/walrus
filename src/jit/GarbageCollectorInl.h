...
static sljit_sw copyArray(GCArray* dstArray, uint32_t dstOffset, GCArray* srcArray, GCArrayCopyArguments* args)
{
    ...
    uintptr_t mask = (static_cast<uintptr_t>(1) << 0) - 1;
    uint8_t* dstAddr = reinterpret_cast<uint8_t*>(dstArray) + ((sizeof(GCArray) + mask) & ~mask) + dstOffset;
    uint8_t* srcAddr = reinterpret_cast<uint8_t*>(srcArray) + ((sizeof(GCArray) + mask) & ~mask) + args->srcOffset;
